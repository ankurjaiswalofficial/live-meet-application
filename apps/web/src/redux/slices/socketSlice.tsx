...
const generateInstantMeet = createAsyncThunk(
    ...
    async (
        ...
        const state = getState() as {
            socketHandler: SocketSliceStates;
            userHandler: UserSliceStates;
        };
        if (wsRef?.current) {
        ...
        peerConnectionRef.current.onicecandidate = (event) => {
            if (event.candidate) {
                wsRef.current?.send(
                    JSON.stringify({
                        type: "signal",
                        meetId: state.socketHandler.meetId,
    ...
    async (
        {
            socket,
            event,
        }: { socket: WebSocket; event: RTCPeerConnectionIceEvent },
        { getState, dispatch }
    ) => {
        if (event.candidate) {
            socket.send(
...
const joinMeetRoom = createAsyncThunk(
    ...
    async (
        { wsRef }: { wsRef: MutableRefObject<WebSocket | null> },
        { getState }
    ) => {
